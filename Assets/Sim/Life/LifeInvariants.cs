    ...
    public static class LifeInvariants
    {
        ...
        public static MetricCollector Collector()
        {
            return (world, into) =>
            {
                ...
                long outside = 0, badLevels = 0, alive = 0;
                ...
                for (int i = 0; i < c.Length; i++)
                {
                    ...
                    if (c.Band[i] >= 0) { int n; members.TryGetValue(c.Band[i], out n); members[c.Band[i]] = n + 1; }
                }
                long miscounted = alive == c.Count ? 0 : 1;
                foreach (Band b in life.Bands)
                {
                    int n; members.TryGetValue(b.Number, out n);
                    if (n != b.Members) miscounted++;
                }
        ...
        public static IReadOnlyList<Invariant> All()
        {
            return new List<Invariant>
            {
                ...
                Invariant.PerRun("M1", "the living are counted right, and every band knows its members",
