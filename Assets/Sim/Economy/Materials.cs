...
namespace Godless.Sim.Economy
{
    ...
    public sealed class Catchment
    {
        ...
        public int Harvest(int material, double labourTicks, MaterialStock stock, VoxelWorld world, long tick,
                           ...
                           System.Action<int, int, int, int, RecordId> cutAt = null)
        {
            ...
            // Where there is anyone to carry it, what was cut lies by the stump:
            // the caller says where it goes (x, z, material, voxels, cause).
            if (taken > 0)
            {
                if (cutAt != null) cutAt(_deposits.X(feature), _deposits.Z(feature), material, taken, _worked[material]);
